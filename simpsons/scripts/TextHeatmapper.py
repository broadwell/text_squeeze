...
labels = [line.rstrip('\n').replace(' ', '_') for line in open(labelsfile, 'r')]
print("number of labels: " + str(len(labels)))

classes = [line.rstrip('\n').replace(' ', '_') for line in open(classesfile, 'r')]
...
# If we can't show every label on the Y axis, this skips some of them
labelsToSkip = 5
#axisLabels = labels
...
if (os.path.isfile('LDAmodel.pkl') and (useCachedfiles == True)):
  ...
  model = gensim.models.ldamodel.LdaModel(corpus=dict_corpus, id2word=kt_dict, num_topics=50)
...
docs = { 'label': classes, 'cluster': clusters, 'collection': collections }
